...
Figure~\ref{fig:tsquared_rise}. As previously noted, the use of a strict prior
significantly reduces the flexibility of the model. As a result, there are far
fewer SNe with model parameters that can immediately be considered unreliable.
When using the $\alpha = 2$ prior, we only flag SNe with an extrapolated flux
using the maximum aposteriori model parameters $< 0.9 f_\mathrm{max}$ at \tbmax\
as having unreliable model parameters. Using this criterion we find \todo{add
number of reliable SNe} SNe have unreliable model parameters.
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{./figures/trise_z_tsquared.pdf}
    %
    \caption{}
    %
