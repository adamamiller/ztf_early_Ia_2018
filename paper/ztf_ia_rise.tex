...
Figure ~\ref{fig:model_parameters}. We show \trise, as opposed to \tfl, to
account for the uncertainty on the measured time of maximum light in the
$B$-band, $t_\mathrm{B,max}$. To do this we approximate the probability
density function (PDF) for \tfl\ via a Gaussian kernel density estimation
(KDE), where the kernel width is determined via cross validation with
...
$t_\mathrm{B,max}$ to arrive at the final PDF for \trise. The cumulative
...
on \tfl\ and $t_\mathrm{B,max}$, as these estimates are made from different
data, which is why we can convolve the uncertainties in making the final
estimation of \trise.
