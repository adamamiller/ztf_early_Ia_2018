...
Of the 127 SNe Ia in our sample, we find that the MCMC chains do not converge
for 15 SNe, as summarized in Table~\ref{tab:no_results}. Most of these SNe
feature gaps in coverage during the critical portion of the photometric
evolution: when the SN is increasing in brightness from 0 to 40\% of the peak
flux. For the remaining 112 SNe the MCMC chains converge, thereby providing
...
\tfl. Roughly, the good models have $\mathrm{CR}_{90} < 1$\,d, the median
models have $\mathrm{CR}_{90} \approx 2$\,d, and the poor models have
$\mathrm{CR}_{90} \ga 4$\,d. From the corner plots it is clear that there is
a very strong covariance between $\alpha$ and $A$. Also, as might be
expected, there is a positive correlation between $\alpha_g$ and $\alpha_r$,
...
covariance with each of the $\alpha$ and $A$ parameters. While we report marginalized confidence intervals on all the model parameters below, we note that the full posterior samples should be used for any downstream analysis given the strong covariance between the parameters.
...
\begin{figure}
    ...
    the median estimate and the 68\% credible region (shown with vertical
    dashed lines).}
    %
...
\begin{figure}
    ...
    poor model constraints feature a multimodal posterior for \tfl, typically
    due to a small gap in observations around the expected time of first
    light. }
    %
