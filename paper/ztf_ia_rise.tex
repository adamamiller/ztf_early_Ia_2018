...
in many SNe (\S\ref{XXX}; see also Paper III in this series, Bhulla et al.\
2019, submitted). }
...
In adopting two very different priors that, in turn, produce significantly different posteriors, we are naturally left with the question:  which model is better? To some extent, the answer to this question rests with every individual as the prior quantifies a priori beliefs about the model parameters. We  posit that  it is extremely unlikely that thermonuclear explosions all identically produce $\alpha = 2$

We can address the notion of which model  is  better by comparing  the model parameters  in a 
