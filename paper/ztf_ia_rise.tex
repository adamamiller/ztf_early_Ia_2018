...
sample, we can still constrain the rate of such events using binomial
...
\begin{figure*}
    ...
    and the thin gray line is not visible)}. The bottom panel shows residuals
    normalized by their uncertainties (pull) relative to the maximum a
    posteriori model, where the factor $\beta_d$ has been included in the
...
in a SN photosphere at frequencies $\lesssim 10^{15}$\,Hz are dominated by
Thomson scattering \citep[see Figure~6 in][]{Magee18}. Furthermore, the
significant overlap between the $B$- and \gztf-bands suggests that \tfl\
should be extremely similar, if not identical, in these two filters.} } An
estimate of \trise\ must therefore account for the uncertainties on both \tfl\
and \tbmax. \citet{Aldering00} critically showed that ignoring the
uncertainties on the time of maximum could lead to \trise\ estimates that are
incorrect by $\ga 2$\,d.
...
full moon. These two effects would contribute additional noise, delaying the
...
this data set are truly unique, making this sample of ZTF SNe the premier
...
\citealt{Yao19}). \edit1{In Appendix~\ref{sec:rare}, we find that the rise
...
ZTF18abclfee (SN\,2018cxk), an 02cx-like SN with exquisite observational
coverage in the time before explosion. We estimate the \tfl\ to within
