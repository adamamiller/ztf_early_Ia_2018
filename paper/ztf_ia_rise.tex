...
excluded from the full sample analysis described in \S\ref{sec:analysis}
below (see Appendix~\ref{sec:qa}). These flagged sources are noted in
Table~\ref{tab:results}.

\section{Sample Analysis \label{sec:analysis}}

Using the marginalized posteriors from the model fits, we can search for
population-level correlations between the model parameters. These
correlations may reveal details about the explosion physics of SNe Ia (for
example, if strong mixing in the SN ejecta leads to low values of $\alpha$,
as found in \citealt{Piro16}, then any correlations with $\alpha$ may be
related to ejecta mixing). Alternatively, these correlations may reveal
inadequacies in the model. We can additionally search for correlations
between the model parameters and redshift, which, if present, could reveal
cosmic evolution of SNe Ia progenitors.

The model given in Equation~\ref{eqn:flux_model} constrains \tfl, yet we
ultimately care about the rise time \trise. \tfl\ is measured relative to
\tbmax, the time of maximum light in the $B$-band, which itself has some
measurement uncertainty. An estimate of \trise\ must therefore account for
the uncertainties on both \tfl\ and \tbmax. To estimate \trise, we use a
Gaussian kernel density estimation (KDE) to approximate the 1-D marginalized
...
The correlation between $t_\mathrm{rise}$, $\alpha_g$, $\alpha_r$, and $z$
is shown in Figure ~\ref{fig:model_parameters}. We do not show the
correlation between $\alpha_r$ and $z$ or between $\alpha_g$ and \trise, as
this information is effectively redundant given the tight correlation
between $\alpha_g$ and $\alpha_r$ (top right panel). We highlight three
subsets of the sample in Figure~\ref{fig:model_parameters}, SNe with
reliable model parameters (see Appendix~\ref{sec:qa}) and known host galaxy
redshifts (hereafter the reliable-$z_\mathrm{host}$ group), SNe with
reliable model parameters and unknown host galaxy redshifts (hereafter the
reliable-$z_\mathrm{SN}$ group), and SNe with unreliable model parameters
(hereafter the unreliable group).
...
\subsection{Correlations with Redshift -- Systematics, Not Cosmic Evolution}

While less prominent, Figure~\ref{fig:model_parameters} additionally shows
that both \trise\ and $\alpha$ are correlated with redshift. This result
is somewhat surprising as it suggests some form of cosmic evolution in SNe
...
sample, and several previous studies (e.g., \todo{Riess ref on Ia
evolution}), render this naive explanation in doubt. Instead, this
correlation can be understood as the result of building a sample from a
flux-limited survey.
...
We illustrate the behavior described in the previous paragraph in
Figure~\ref{fig:high_z_systematic}, which shows the inferred rise time for
four low-$z$ SNe that have been shifted to successively higher redshifts. To
simulate these SNe at higher redshift we make the (over-simplified) assumption
that all detections are in the sky-background dominated regime, and therefore
in any given epoch the $\mathrm{SNR} \propto d_L^2$, where $d_L$ is the
luminosity distance. Under this assumption, multiplying the uncertainties by
the ratio of the simulated distance to the actual distance squared provides
...
\citep{Amanullah10} to calculate luminosity distances to the SNe.} Using these
increased uncertainties, we randomly resample the observed flux values from a
normal distribution with mean equal to the original flux and variance equal to
the scaled uncertainty squared. This resampling is necessary, as the $\beta_d$
...
\begin{figure}
    ...
    an observational systematic, namely a $\sim$fixed detection threshold,
    and not cosmic evolution.}
    %
...
\todo{Well worded paragraph on the implications of this result - the results are quite meaningful}
...
\begin{figure*}
    ...
    Figure~\ref{fig:model_parameters}. \trise\ shows a strong correlation
    with $X_1$, while there is no correlation between $\alpha_g$ and $X_1$.
    The second, third, and fourth columns highlight SNe with redshifts $z \le
    0.04$, $0.04 < z \le 0.065$, and $z > 0.065$, respectively. Dividing the
    sample into different redshift bins shows that most of the observed
    scatter in the first column is due to redshift, and not intrinsic scatter
    in the correlation between \trise\ and $X_1$. }
    %
    \label{fig:high_z_systematic}
\end{figure*}
...
\begin{figure}
    ...
    SNe with reliable fits with known host galaxy redshifts, dark blue
    lines show reliable fits where the host galaxy redshift is not known,
    and thin, light grey lines show SNe with unreliable fits). From the
...
\begin{figure}
    ...
    index in the \gztf-band $\alpha_g$ assuming a flat pqrior on $\alpha_åg$
    for individual SNe in our sample. The color scheme is the same as in
    Figures~\ref{fig:model_parameters}~and~\ref{fig:rise_time}. While the
...
As noted in \S\ref{sec:model}, the MCMC model converges for all but one
ZTF SNe within the sample. However, visual inspection of both the corner
plots and individual draws from the posterior quickly reveals that for some SNe the data do not provide strong constraints on the model parameters (see for example Figures~\ref{fig:bad_corner} and~\ref{fig:bad_lc}).
In the most extreme cases, as shown in Figure~\ref{fig:biggap_lc}, large gaps
in the observations make it nearly impossible to constrain the model
...
\begin{figure}
    ...
    %
    \caption{Same as Figure~\ref{fig:good_lc} for ZTF18aaqffyp, a SN with
